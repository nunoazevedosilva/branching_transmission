...
class vz_distribution:
    
    def __init__(self, simulation_configuration,power,Nz):
        ...
        self.af_array = af.constant(0., simulation_configuration.Nx, simulation_configuration.Ny, 
                                    simulation_configuration.Nt, Nz, af.Dtype.c64)
    ...
    def save_field(self, index):
        ...
        key="NL"
        af.array.save_array(key, self.af_array, "config_folder/Vector_z"+".af")
...
class simulation_config:
    ...
    """
    def __init__(self, dims, Nx, Ny, Nt, Nz, total_steps, stride,dx,dy,dz,dt,number_nl_numbers, number_nl_vectors, vz_distribution, saveDir):
        ...
        self.vz_distribution = vz_distribution
    ...
    def gen_config_file(self,nl_numbers,nl_vectors):
        ...
        L0= [self.dims, self.Nx, self.Ny, self.Nt, self.Nz, 
            ...
            self.dt,self.number_nl_numbers, self.number_nl_vectors, self.vz_distribution, self.saveDir]
        L = [str(i)+'\n' for i in L0]
        try:
            os.mkdir('config_folder')
        except:
    ...
    def plot(self,nl_vectors=[]):
        ...
        lines = 1 + 1 + int((self.number_nl_vectors-1)/2)
        ...
        for l in range(0, self.number_nl_vectors):
            ...
            ax=fig.add_subplot(lines, 2, 3+2*int(l/2)+int(l)%2)
